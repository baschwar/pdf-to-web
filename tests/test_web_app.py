...
class WebAppTests(unittest.TestCase):
    ...
    def test_structure_exposes_source_region_and_page_dimensions(self, page_size):
        ...
        self.assertIn('id="selected-block-page">Page -</span>', response.text)
        self.assertIn('id="selected-block-position"', response.text)
        self.assertIn('id="next-block"', response.text)
        self.assertNotIn('<span class="block-meta"><span>Page 1</span>', response.text)
        self.assertIn('<span class="block-status status-unreviewed">Unreviewed</span>', response.text)
    ...
    def test_local_image_preview_and_export_media_summary(self):
        ...
        original_path(self.project).write_text(json.dumps(document))
        structure = self.client.get("/structure")
        self.assertIn('class="block-form image-block-form"', structure.text)
        self.assertIn('name="alt"', structure.text)
        self.assertIn('name="caption"', structure.text)
        self.assertIn('name="decorative"', structure.text)
        edited_image = self.client.post("/api/blocks/image", headers=self.headers(), json={"alt": "Updated login screen", "caption": "Updated sign in", "decorative": False, "review_status": "approved"})
